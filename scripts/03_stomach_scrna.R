...
## Path to the Seurat object downloaded from CZ CELLxGENE (see URL in comment above)
scrna_stomach_rds <- "data/stomach_scrna.rds"
...
## Read seurat object
nowicki <- readRDS(scrna_stomach_rds)
nowicki <- SetIdent(nowicki, value = nowicki@meta.data$Celltypes_global)

## Subset to only healthy tissue
nowicki_healthy <- subset(
  nowicki,
  subset = Tissue_in_paper %in%
    c("NE", "SMG", "NSCJ", "NGB", "NGC", "ND", "Ileum", "Colon", "Rectum")
)
...
DimPlot(
  nowicki,
  reduction = ".umap_MinDist_0.2_N_Neighbors_15",
  label = TRUE,
  label.size = 3,
  raster = T
) +
...
## Define the function
compute_gene_correlation <- function(
  seurat_obj,
  celltype,
  target_gene = "ENSG00000109511"
) {
  cell_subset <- subset(seurat_obj, subset = Celltypes_global %in% celltype)
  selected_genes <- rownames(cell_subset)[
    Matrix::rowSums(cell_subset@assays$RNA@counts) > 0
  ]
...
foveolar_int.corr <- compute_gene_correlation(
  nowicki_healthy,
  celltype = "Foveolar_Intermediate"
)
foveolar_diff.corr <- compute_gene_correlation(
  nowicki_healthy,
  celltype = "Foveolar_Differentiated"
)
neck.corr <- compute_gene_correlation(nowicki_healthy, celltype = "Neck-Cells")
parietal.corr <- compute_gene_correlation(
  nowicki_healthy,
  celltype = "Parietal"
)
healthy.corr <- compute_gene_correlation(
  nowicki_healthy,
  celltype = unique(nowicki_healthy@meta.data$Celltypes_global)
)

## join together
anxa.corr <- healthy.corr %>%
  full_join(
    foveolar_diff.corr,
    by = "gene",
    suffix = c("", ".foveolar_diff")
  ) %>%
...
corr.quantiles <- anxa.corr.long %>%
  group_by(variable) %>%
  summarise(
    quantile95 = quantile(value, probs = 0.95, na.rm = T),
...
ogtt1 <- ogtt1 %>%
  group_by(UniProt) %>%
  slice(which.min(fdr.aov)) %>%
  summarize(fdr.aov = first(fdr.aov))
ogtt2 <- ogtt2 %>%
  group_by(UniProt) %>%
  slice(which.min(fdr.aov)) %>%
  summarize(fdr.aov = first(fdr.aov))
...
ogtt.both$sig <- ifelse(
  ogtt.both$ogtt1.fdr.aov < 0.05 | ogtt.both$ogtt2.fdr.aov < 0.05, 1, 0)

gene_list <- getBM(
  filters = "uniprotswissprot",
  attributes = c("uniprotswissprot", "ensembl_gene_id", "hgnc_symbol"),
  values = ogtt.both$UniProt,
  mart = ensembl
)
ogtt.both <- ogtt.both %>%
  left_join(gene_list, by = c("UniProt" = "uniprotswissprot"))

anxa.corr.ogtt <- anxa.corr %>%
  full_join(ogtt.both, by = c("gene" = "ensembl_gene_id"))

anxa.corr.ogtt$coexp_healthy <- ifelse(
  anxa.corr.ogtt$healthy.cor > quantiles[grepl("healthy", names(quantiles))],1,0)
...
percentage_expressing <- anxa10_expression %>%
  group_by(cell_type) %>%
  summarize(percent_expressing = sum(ENSG00000109511 > 0) / n() * 100)

# Sort cell types by percentage
sorted_cell_types <- percentage_expressing %>%
  arrange(desc(percent_expressing)) %>%
  pull(cell_type)

# Order cell types in the Seurat object
nowicki_healthy <- SetIdent(
  nowicki_healthy,
  value = factor(Idents(nowicki_healthy), levels = rev(sorted_cell_types))
)

top25_corr <- head(
  anxa.corr.ogtt[order(-anxa.corr.ogtt$healthy.cor), ]$gene,
  25
)
top25_corr_names <- head(
  anxa.corr.ogtt[
    order(-anxa.corr.ogtt$healthy.cor),
  ]$hgnc_symbol.x,
  25
)

## Dotplot with top 25 coexpressed genes, and CA9 and PLA2G10
DotPlot(
  nowicki_healthy,
  features = c(top25_corr, "ENSG00000107159", "ENSG00000069764")
) +
...
## Dotplot with only genes and cell types of interest 
DotPlot(
  nowicki_healthy,
  features = genes,
  idents = c(
