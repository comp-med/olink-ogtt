...
###################################################
#### Pathway Enrichment with Gprofiler         ####
###################################################

## Define gene sets
ogtt1_sig <- ogtt.both %>% filter(fdr.aov.1 < .05) %>% pull(Assay) %>% unique()
ogtt2_sig <- ogtt.both %>% filter(fdr.aov.2 < .05) %>% pull(Assay) %>% unique()

ogtt_consistent <- ogtt.both %>%
  filter(
    fdr.aov.1 < 0.05 &
      pval.aov.t.factor.2 < 0.05 |
      fdr.aov.2 < 0.05 & pval.aov.t.factor.1 < 0.05
  ) %>%
  pull(Assay) %>%
  unique()

olink_universe <- ogtt.both %>% pull(Assay) %>% unique()
...
test_enrichment <- function(geneset, universe, label) {
  ...
  res.enrich$OR <- res.enrich$intersection_size *
    (res.enrich$effective_domain_size +
      res.enrich$term_size +
      res.enrich$query_size -
      res.enrich$intersection_size * 2) /
    ((res.enrich$query_size - res.enrich$intersection_size) *
      (res.enrich$term_size - res.enrich$intersection_size))
...
res.enrich.kegg_reac$source <- factor(
  res.enrich.kegg_reac$source,
  levels = c("KEGG", "REAC")
)
...
write.csv(
  pruned_results,
  "output/enriched_pathways_pruned.csv",
  row.names = F
)
...
tmp <- pruned_results %>%
  filter(geneset %in% c("OGTT 1 only", "OGTT 2 only", "Consistent in both"))
...
gp <- ggplot(
  tmp,
  aes(x = log10p, y = OR, shape = as.factor(source), col = geneset)
) +
  geom_point(size = 4) +
  geom_text_repel(
    data = hl_terms,
    aes(label = wrapped_term, col = geneset),
    size = 4
  ) +
  geom_vline(
    xintercept = -log10(.05),
  ...
  theme_minimal() +
  labs(x = "-log10(p-adj)", shape = "Source") +
  theme(
    panel.spacing = unit(1, "lines"),
