...
## Excel with glucose, insulin, ffa, BUT measurements during OGTTs
ogtt.dat <- data.frame(read_excel(
  "data/clinical_data.xlsx"
))
...
ogtt.dat_long$biomarker <- gsub(
  "ffa",
  "free fatty acids",
  ogtt.dat_long$biomarker
)

## create time also as factor
ogtt.dat_long$ogtt.session <- as.factor(ogtt.dat_long$ogtt.session)
ogtt.dat_long$t.factor <- as.factor(ogtt.dat_long$t.point)
...
ogtt.summ <- ogtt.summ <- ogtt.dat_long %>%
  group_by(ogtt.session, t.point, biomarker) %>%
  summarise(
...
write.table(
  ogtt.summ,
  "output/clinical_mean_and_sd.txt",
  sep = "\t",
  row.names = F
)
...
## get participant sex
ogtt.dat_long <- ogtt.dat_long %>% left_join(participant, by = "participant")
ogtt.dat_long$sex <- as.factor(ogtt.dat_long$sex)

## Diff between OGTT 1 and 2

#glucose
res.gluc.diff <- mixed.regression.v2(
  subset(ogtt.dat_long, biomarker == "glucose"),
  "measurement",
  paste("ogtt.session*t.factor + (1|participant)"),
  "inter",
  log.y = F,
  scale = F
)
res.gluc.diff <- do.call(rbind, res.gluc.diff) %>% as.data.frame()

# insulin
res.ins.diff <- mixed.regression.v2(
  subset(ogtt.dat_long, biomarker == "insulin"),
  "measurement",
  paste("ogtt.session*t.factor + (1|participant)"),
  "inter",
  log.y = F,
...
res.ins.diff <- do.call(rbind, res.ins.diff) %>% as.data.frame()

res.diff <- cbind(res.gluc.diff, res.ins.diff)
colnames(res.diff) <- c("glucose", "insulin")

## Sex differential effects
res.sex.all <- list()

for (sess in c(1, 2)) {
  for (bio in c("glucose", "insulin")) {
    cat("\nRunning:", bio, "session", sess, "\n")

    res.tmp <- mixed.regression.v2(
      subset(ogtt.dat_long, biomarker == bio & ogtt.session == sess),
      "measurement",
      "sex*t.factor + (1|participant)",
      paste0(bio, "_s", sess),
      log.y = FALSE,
      scale = FALSE
    )

    res.tmp <- do.call(rbind, res.tmp) %>% as.data.frame()

    # extract the ANOVA p-value for sex × time
    pval <- res.tmp["pvalue.anova_sex.t.factor", ]

    res.sex.all[[paste0(bio, "_s", sess)]] <- data.frame(
      biomarker = bio,
      session = sess,
      pval = pval
    )
...
res.sex.all <- do.call(rbind, res.sex.all)
res.sex.all$fdr <- p.adjust(res.sex.all$pval, method = "BH")

################## Convert units ###############################################

ogtt.dat_long2$unit[ogtt.dat_long2$biomarker == "insulin"] <- "pmol/l"

ogtt.dat_long2$unit[ogtt.dat_long2$biomarker == "glucose"] <- "mmol/l"
...
## Participants excluded due to missing proteomics data in one or both OGTT sessions
participants_missing_proteomics <- c(participant_ID1, participant_ID2) # ids are not real
exclude <- participants_missing_proteomics

ci_plots <- list()

for (b in biomarkers) {
  mean_data <- ogtt.dat_long2 %>%
    filter(
      biomarker == b,
      !is.na(measurement),
    ...
    summarize(
      ...
      se = qt(0.975, df = sum(!is.na(measurement)) - 1) *
        sd(measurement, na.rm = TRUE) /
        sqrt(sum(!is.na(measurement))),
  ...
  p <- ggplot(mean_data, aes(x = t.point, y = mean_value, col = ogtt.session)) +
    ...
    theme_classic() +
    labs(x = "Time [min]", y= paste0(str_to_title(b), " [", unit, "]"), col = "OGTT session") +
    #ggtitle(paste0(str_to_title(b), " [", unit, "]")) +
    theme(
...
fasting.dat <- data.frame(read_excel(
  "data/participant_data.xlsx"
))
fasting.dat <- fasting.dat %>%
  dplyr::filter(!participant %in% participants_missing_proteomics)
...
fasting.dat$homa_b <- (20 * fasting.dat$insulin_uU_ml) /
  (fasting.dat$glucose - 3.5)
fasting.dat$quicki <- 1 /
  (log10(fasting.dat$insulin_uU_ml) + log10(fasting.dat$glucose_mg_dl))
...
ogtt.dat <- subset(
  ogtt.dat,
  !participant %in% participants_missing_proteomics & biomarker %in% c("glucose", "insulin")
) ## exclude participant w/out ogtt

## Pivot wider to calculate indices per row
names(ogtt.dat)[2:8] <- sub("^X", "", names(ogtt.dat)[2:8])
ogtt.dat <- ogtt.dat[, -11] %>%
  pivot_wider(
    id_cols = c("participant", "ogtt.session"),
...
ogtt.dat$ins_uU_ml_mean <- rowMeans(ogtt.dat[, c(
  "insulin0_uU_ml",
  "insulin30_uU_ml",
  "insulin60_uU_ml",
  "insulin90_uU_ml",
...
ogtt.dat$gluc_mg_dl_mean <- rowMeans(ogtt.dat[, c(
  "glucose0_mg_dl",
  "glucose30_mg_dl",
  "glucose60_mg_dl",
  "glucose90_mg_dl",
...
ogtt.dat$isi_0_120 <- 0.156 -
  0.0000459 * ogtt.dat$insulin120 -
  0.000321 * ogtt.dat$insulin0 -
  0.00541 * ogtt.dat$glucose120
ogtt.dat$isi_0_60 <- 0.149 -
  0.000467 * ogtt.dat$insulin0 -
  0.00466 * ogtt.dat$glucose60

# Metabolic clearance rate (ml · kg–1 · min–1)
ogtt.dat$mcr_0_120 <- 13.273 -
  0.00384 * ogtt.dat$insulin120 -
  0.0232 * ogtt.dat$insulin0 -
  0.463 * ogtt.dat$glucose120
ogtt.dat$mcr_0_60 <- 12.464 -
  0.0357 * ogtt.dat$insulin0 -
  0.376 * ogtt.dat$glucose60

# Stumvoll 1st phase
ogtt.dat$ph1_0_30 <- 1283 +
  1.829 * ogtt.dat$insulin30 -
  138.7 * ogtt.dat$glucose30 +
  3.772 * ogtt.dat$insulin0
ogtt.dat$ph1_0_60 <- 1194 +
  4.724 * ogtt.dat$insulin0 -
  117.0 * ogtt.dat$glucose60 +
  1.414 * ogtt.dat$insulin60

# Stumvoll 2nd phase
ogtt.dat$ph2_0_30 <- 286 +
  0.416 * ogtt.dat$insulin30 -
  25.94 * ogtt.dat$glucose30 +
  0.926 * ogtt.dat$insulin0
ogtt.dat$ph2_0_60 <- 295 +
  0.349 * ogtt.dat$insulin60 -
  25.72 * ogtt.dat$glucose60 +
  1.107 * ogtt.dat$insulin0


################## AUC and incremental AUC #####################################

## AUC minutes 0 to 120
ogtt.dat$auc_gluc_0_120 <- 0.125 *
  ogtt.dat$glucose0 +
  0.25 * ogtt.dat$glucose15 +
  0.375 * ogtt.dat$glucose30 +
  0.5 * ogtt.dat$glucose60 +
  0.5 * ogtt.dat$glucose90 +
  0.25 * ogtt.dat$glucose120
...
gluc_nadir <- ogtt.dat[, c(1:14)] %>%
  mutate(
    gluc_nadir_tpoint = case_when(
      ## determine which tpoint glucose not decrease below baseline
...
gluc_nadir <- gluc_nadir %>%
  mutate(across(
    starts_with("glucose"),
    ~ ifelse(
      as.numeric(sub("glucose", "", cur_column())) >
        as.numeric(sub("glucose", "", gluc_nadir_tpoint)),
      0,
      .
    )
  ))
...
ogtt.dat$iauc_gluc_0_120 <- 0.125 *
  gluc_nadir$glucose0 +
...
ogtt.dat$quicki <- 1 /
  (log10(ogtt.dat$insulin0_uU_ml) + log10(ogtt.dat$glucose0_mg_dl))
...
ogtt.dat$auc_ins_0_30 <- 0.125 *
  ogtt.dat$insulin0 +
  0.25 * ogtt.dat$insulin15 +
  0.125 * ogtt.dat$insulin30
ogtt.dat$auc_gluc_0_30 <- 0.125 *
  ogtt.dat$glucose0 +
  0.25 * ogtt.dat$glucose15 +
  0.125 * ogtt.dat$glucose30
...
# insulinogenic index
ogtt.dat$insulinogenic_idx <- (ogtt.dat$insulin30 - ogtt.dat$insulin0) /
  (ogtt.dat$glucose30 - ogtt.dat$glucose0)

# Matsuda index
ogtt.dat$matsuda <- 10000 /
  sqrt(
    ogtt.dat$glucose0_mg_dl *
      ogtt.dat$insulin0_uU_ml *
      ogtt.dat$gluc_mg_dl_mean *
      ogtt.dat$ins_uU_ml_mean
  )
...
ogtt.indices <- ogtt.indices %>%
  pivot_longer(cols = 3:ncol(ogtt.indices), names_to = "index_name") %>%
  pivot_wider(names_from = ogtt.session)
...
sig_indices <- ttest_res %>% filter(pval < (0.05 / 17)) %>% pull(index) # bonferroni correction for 17 indices

ogtt.indices_long <- ogtt.indices %>%
  filter(index_name %in% sig_indices) %>%
  pivot_longer(cols = 3:4, names_to = "ogtt_session", values_to = "value")

facet_titles <- c(
  "ph1_0_60" = "Stumvoll 1st phase",
...
facet_titles <- as.data.frame(tibble(
  index_name = names(facet_titles),
  formatted_name = unlist(facet_titles)
))

ogtt.indices_long <- ogtt.indices_long %>%
  left_join(facet_titles, by = "index_name")

ogtt.indices_long$formatted_name <- factor(
  ogtt.indices_long$formatted_name,
  levels = c(
    "Stumvoll 2nd phase",
    "Stumvoll 1st phase",
    "HIRI",
...
ggplot(
  ogtt.indices_long[
    ogtt.indices_long$index_name %in% c("ph1_0_60", "matsuda"),
  ],
  aes(x = ogtt_session, y = value)
) +
  geom_boxplot() +
  ...
  facet_wrap(
    ...
    scales = "free_y",
    ncol = 2,
    labeller = labeller(idx = facet_titles)
  )

## Plot the rest
ggplot(
  ogtt.indices_long[
    ogtt.indices_long$index_name %in%
      c("ph2_0_60", "hiri", "ifc_0_120", "iauc_gluc_0_120"),
  ],
  aes(x = ogtt_session, y = value)
) +
  geom_boxplot() +
