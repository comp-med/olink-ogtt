...
plot_manhattan <- function(data, snps) {
  ...
  # Draw plot
  plot <- ggplot(gwas_data, aes(x = bp_cum, y = LOG10P,
                                color = as.factor(CHROM))) +
    #geom_hline(yintercept = -log10(sig), color = "grey40", linetype = "dashed") + 
    geom_point(alpha = 0.75, size=0.5) +
    scale_x_continuous(label = axis_set$CHROM, breaks = axis_set$center) +
    scale_y_continuous(expand = c(0,0), limits = c(2, ylim)) +
    scale_color_manual(values = rep(c("#332288", "#6699CC"),
                                    unique(length(axis_set$CHROM)))) +
    labs(x = NULL, y = "-log10P") + 
    geom_text_repel(data = subset(gwas_data, ID %in% snps), aes(label=ID), size=3) +
